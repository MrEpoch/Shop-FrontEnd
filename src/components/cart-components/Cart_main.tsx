...
export default function Cart__main() {
    ...
    <div className="shop__cart_modal">
        <Badge onClick={() => setOpenModal(true)} style={{ padding: "5px" }} badgeContent={Get_cart_length()} color="secondary">
            <CartIcon className="clickable" />
        </Badge>
        <div className={`modal ${openModal ? "is-active" : ""}`}>
          <div onClick={() => setOpenModal(false)} className="modal-background background_style_modal"></div>
          <div className={`modal-card ${theme ? "dark__modal__body" : ""}`}>
            <header className={`modal-card-head ${theme ? "dark__modal__header" : ""}`}>
              <p className="modal-card-title">Shopping Cart</p>
              <button className="delete" onClick={() => setOpenModal(false)} aria-label="close"></button>
            </header>
            <section className={`modal-card-body ${theme ? "dark__modal__body" : ""}`}>
            ...
            <footer className={`modal-card-foot ${theme ? "dark__modal__header" : ""}`}>
              <button onClick={() => setOpenModal(false)} className="button">Continue shopping</button>
